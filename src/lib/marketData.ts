...
function completedOnly(candles: CandlePoint[], cutoff: Date, timeframeMinutes: number): CandlePoint[] {
...
async function collectLive(symbols: string[], dateKey: string, cutoff: Date, opts: CollectOptions): Promise<CollectedMarketData> {
  ...
  // One V3 historical request per symbol supplies today's 5m candles,
  // prior-session PDH/PDL and EMA warmup. This replaces the old three-request
  // per-symbol fan-out that was causing Upstox 429s and Vercel 504s.
  ...
  // LTP is display-only and fetched in one batched V3 request.
  try {
