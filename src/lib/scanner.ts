...
async function runScan(dateKey: string, phase: MarketPhase, forcedCutoff?: Date, batchIndex?: number): Promise<{ error: string | null; message: string | null }> {
  ...
  try { existing = await db.select({ id: snapshots.id, symbol: snapshots.symbol, status: snapshots.status, statusRank: snapshots.statusRank, level: snapshots.level, pdh: snapshots.pdh, pdl: snapshots.pdl, firstConfirmedAt: snapshots.firstConfirmedAt }).from(snapshots).where(eq(snapshots.scanDate, dateKey)); } catch { existing = []; }
  const prevBySymbol = new Map(existing.map((e) => [e.symbol, e]));
  const eventRows: (typeof signalEvents.$inferInsert)[] = [];
  let confirmed = 0, setups = 0, watch = 0;
  ...
  for (const [symbol, row] of fresh) {
    ...
    if (prev && prev.status === "CONFIRMED" && row.statusRank > STATUS_RANK.SETUP) {
      ...
      const stillBeyond = lvl != null && row.ltp != null ? prev.level === "PDH" ? row.ltp > lvl : row.ltp < lvl : true;
      upserts.push({ scanDate: dateKey, symbol, ltp: row.ltp, changePct: row.changePct, active: stillBeyond, updatedAt: now } as typeof snapshots.$inferInsert);
      continue;
    }
    if (row.status === "NONE") {
      if (prev && SIGNAL_STATUSES.has(prev.status)) upserts.push({ scanDate: dateKey, symbol, ltp: row.ltp, changePct: row.changePct, active: false, updatedAt: now } as typeof snapshots.$inferInsert);
      continue;
    }
    const escalated = !prev || row.statusRank < prev.statusRank;
    upserts.push({ scanDate: dateKey, symbol, ltp: row.ltp, changePct: row.changePct, dayOpen: row.dayOpen, volumeMultiple: row.volumeMultiple, volumeLabel: row.volumeLabel, ema20: row.ema20, emaBias: row.emaBias, pdh: row.pdh, pdl: row.pdl, level: row.level, distancePct: row.distancePct, setup: row.setup, status: row.status, direction: row.direction, entry: row.entry, stopLoss: row.stopLoss, target: row.target, reason: row.reason, details: row.details, statusRank: row.statusRank, active: row.active, firstConfirmedAt: row.status === "CONFIRMED" ? prev?.firstConfirmedAt ?? now : prev?.firstConfirmedAt ?? null, updatedAt: now });
    if (escalated && (row.status === "SETUP" || row.status === "CONFIRMED")) {
      ...
      eventRows.push({ scanDate: dateKey, ts: now, timeLabel: signalTime ?? istTimeLabel(now, true), symbol, direction: row.direction, level: row.level, setup: row.setup, status: row.status, price: row.ltp, volumeMultiple: row.volumeMultiple, reason: row.reason });
    }
  }
  ...
  for (const u of upserts) if (u.status) finalState.set(u.symbol, { status: u.status as string, rank: u.statusRank ?? 9 });
  for (const v of finalState.values()) { if (v.status === "CONFIRMED") confirmed++; else if (v.status === "SETUP") setups++; else if (v.status === "WATCH") watch++; }

  const dbWrites = async () => {
    for (const u of upserts) {
      const { scanDate: sd, symbol: sym, ...rest } = u;
      await db.insert(snapshots).values({ ...u, scanDate: sd ?? dateKey, symbol: sym } as typeof snapshots.$inferInsert).onConflictDoUpdate({ target: [snapshots.scanDate, snapshots.symbol], set: rest as Record<string, unknown> });
    }
    if (eventRows.length) await db.insert(signalEvents).values(eventRows);
    await db.insert(scanRuns).values({ scanDate: dateKey, startedAt, finishedAt: new Date(), status: errorCount > 0 ? "PARTIAL" : "OK", source: collected.source, marketPhase: phase, universeSize: UNIVERSE.length, processed, errorCount, confirmed, setups, watch, message: errorCount > 0 ? `${errorCount} symbol(s) failed to load and were skipped.` : null });
  ...
  try { await dbWrites(); } catch (err) { return { error: `Database write failed: ${(err as Error).message}`, message: null }; }
  return { error: null, message: errorCount > 0 ? `Scan complete with ${processed} symbols (${errorCount} skipped).` : null };
}
...
async function loadEventsForDate(dateKey: string): Promise<SignalEventRow[]> {
  try {
    ...
    return rows.map((e) => ({ id: e.id, time: e.timeLabel, symbol: e.symbol, direction: e.direction as Direction, level: e.level as LevelTag, setup: e.setup as SetupType, status: e.status as SignalStatus, price: e.price, volumeMultiple: e.volumeMultiple, reason: e.reason }));
  } catch { return []; }
}

async function latestPersistedDate(): Promise<string | null> {
  try {
    const rows = await db.select({ d: snapshots.scanDate }).from(snapshots).orderBy(desc(snapshots.scanDate)).limit(1);
...
export async function getDashboardPayload(force = false): Promise<ScanPayload> {
  ...
  if (phase === "LIVE_SCAN") {
    ...
    if (!due) {
      try {
        const rows = await db.select({ startedAt: scanRuns.startedAt }).from(scanRuns).where(eq(scanRuns.scanDate, dateKey)).orderBy(desc(scanRuns.id)).limit(1);
  ...
  if (phase !== "LIVE_SCAN" && rows.length === 0 && source === "SIMULATION") {
    ranScan = true;
    const forced = epochForIst(dateKey, SCANNER_CONFIG.scanEnd);
    const result = await runScan(dateKey, phase, forced);
    error = result.error;
    rows = await loadRowsForDate(dateKey);
    message = phase === "CLOSED" ? `Market closed — simulation replay of the full ${SCANNER_CONFIG.scanStart}–${SCANNER_CONFIG.scanEnd} IST window. Set UPSTOX_ACCESS_TOKEN for live data.` : `Simulation replay of the ${SCANNER_CONFIG.scanStart}–${SCANNER_CONFIG.scanEnd} IST window (${phase === "PRE_OPEN" ? "market opens later" : "scan window ended"}). Set UPSTOX_ACCESS_TOKEN for live data.`;
  }
  let lastScanDate = dateKey;
  if (rows.length === 0 && phase !== "LIVE_SCAN") {
    const persisted = await latestPersistedDate();
    if (persisted && persisted !== dateKey) { rows = await loadRowsForDate(persisted); effectiveDate = persisted; lastScanDate = persisted; }
  }
  ...
  for (const r of rows) { if (r.status === "CONFIRMED") counts.confirmed++; else if (r.status === "SETUP") counts.setups++; else if (r.status === "WATCH") counts.watch++; }
  return { meta: { ok: true, scanDate: effectiveDate, nowIst, marketPhase: phase, source: (runInfo.source as "UPSTOX" | "SIMULATION") ?? source, ranScan, lastScanAt: runInfo.at, lastScanDate, lastScanStatus: runInfo.status, universeSize: UNIVERSE.length, processed: runInfo.processed, errorCount: runInfo.errorCount, counts, message, error, config: { scanStart: SCANNER_CONFIG.scanStart, scanEnd: SCANNER_CONFIG.scanEnd, rescanSeconds: SCANNER_CONFIG.rescanSeconds, volRefCandles: SCANNER_CONFIG.volumeRefCandles, breakoutVolMin: SCANNER_CONFIG.breakoutVolMin, riskReward: SCANNER_CONFIG.riskReward } }, rows, events };
}
