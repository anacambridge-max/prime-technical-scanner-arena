...
import { Activity, Clock3, Database, RefreshCw, Search, Zap } from "lucide-react";
...
export default function MultiTimeframeDashboard() {
  ...
  useEffect(() => {
    const id = setInterval(() => load(false), poll * 1000);
  ...
  return (
    <div className="min-h-screen bg-[#04070d] text-slate-200">
      <header className="sticky top-0 z-40 border-b border-[#16203a] bg-[#04070d]/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1680px] flex-wrap items-center gap-4 px-4 py-3 lg:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[15px] font-bold tracking-[0.14em]">PRIME TECHNICAL <span className="text-emerald-300">MULTI-TIMEFRAME SCANNER</span></h1>
              <p className="mono text-[9px] uppercase tracking-[0.22em] text-slate-500">PDH/PDL · 20 EMA · 1.5x VOLUME · PRIME QUALITY · F&O ONLY</p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-[1680px] space-y-4 px-4 py-4 lg:px-6">
        ...
        <section className="rounded-xl border border-[#1b2537] bg-[#0a101c]/80 p-2">
          <div className="grid grid-cols-3 gap-2">
            {tabs.map(tf => {
              const r = payload?.timeframes?.[String(tf) as "1" | "3" | "5"];
              const selected = active === tf;
              return <button key={tf} onClick={() => setActive(tf)} className={`rounded-lg border px-4 py-3 text-left transition ${selected ? "border-sky-400/50 bg-sky-400/10" : "border-[#202b40] bg-[#0b1220] hover:border-slate-500"}`}>
                <div className="flex items-center justify-between"><span className={`text-[13px] font-bold tracking-widest ${selected ? "text-sky-300" : "text-slate-300"}`}>{label(tf)}</span><span className="mono text-[10px] text-slate-500">{r?.processed ?? 0} stocks</span></div>
                <div className="mt-2 flex gap-3 text-[10px]"><span className="text-emerald-300">CONF {r?.counts.confirmed ?? 0}</span><span className="text-amber-300">SET {r?.counts.setups ?? 0}</span><span className="text-sky-300">WATCH {r?.counts.watch ?? 0}</span></div>
              </button>;
            })}
          </div>
        </section>
        ...
        <ScannerTable rows={rows} expanded={new Set()} onToggle={() => undefined} />
