...
export default function MultiTimeframeDashboard() {
  ...
  // A full initial F&O scan is intentionally paced to respect Upstox's
  // minute quota. Do not poll every few seconds: on Vercel, consecutive
  // requests can land on different serverless instances and accidentally
  // start duplicate 206-stock scans against the same Upstox token.
  const initialScanRunning = !payload?.meta.lastScanAt || payload?.meta.ranScan === false;
  const poll = initialScanRunning ? 30 : Math.min(60, Math.max(30, payload.meta.config.rescanSeconds ?? 45));
